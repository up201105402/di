...
<template>
    <table>
        <thead>
            <tr>
                <th>{{ $t('pages.testers.table.headers.id') }}</th>
                <th>{{ $t('pages.testers.table.headers.name') }}</th>
                <th>{{ $t('pages.datasets.table.headers.path') }}</th>
                <th>{{ $t('pages.testers.table.headers.modified') }}</th>
                <th>{{ $t('pages.testers.table.headers.created') }}</th>
                <th />
            </tr>
        </thead>
        <tbody>
            <tr v-for="model in itemsPaginated" :key="model.id">
                <td :data-label="$t('pages.testers.table.headers.name')">
                    {{ model.ID }}
                </td>
                <td :data-label="$t('pages.testers.table.headers.name')">
                    {{ model.name }}
                </td>
                <td :data-label="$t('pages.testers.table.headers.path')">
                ...
                <td :data-label="$t('pages.testers.table.headers.modified')" class="lg:w-1 whitespace-nowrap">
                    <small class="text-gray-500 dark:text-slate-400" :title="formatDate(model.UpdatedAt)">{{ formatDate(model.UpdatedAt) }}</small>
                </td>
                <td :data-label="$t('pages.testers.table.headers.created')" class="lg:w-1 whitespace-nowrap">
