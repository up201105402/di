...
<template>
  <LayoutAuthenticated>
    <SectionMain>
      ...
      <SectionTitleLineWithButton :hasButton="false" :icon="mdiChartTimelineVariant" :title="$t('pages.pipelines.header')"
        main>
        <BaseButton :icon="mdiPlus" color="success" @click="onNewPipelineClicked" />
      </SectionTitleLineWithButton>

      <PipelinesTable :items="pipelines" @deleteButtonClicked="onDeletePipelineClicked" />
    </SectionMain>

    <CardBoxModal v-model="isCreateModalActive" @confirm="createPipeline(200, createPipelineForm.name)"
      :title="$t('pages.pipelines.dialog.create.header')" button="success" has-cancel>
      <FormField :label="$t('pages.pipelines.dialog.create.name.label')"
        :help="$t('pages.pipelines.dialog.create.name.help')">
        <FormControl v-model="createPipelineForm.name" name="name" autocomplete="name" placeholder="Name"
          :focus="isCreateModalActive" />
      </FormField>
    </CardBoxModal>

    <CardBoxModal v-model="isDeleteModalActive" :title="$t('pages.pipelines.dialog.delete.header')"
      :target-id="pipelineIdToDelete" @confirm="deletePipeline(200, pipelineIdToDelete)" button="danger" has-cancel>
