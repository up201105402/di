...
const { isLoading: isFetching, state: fetchResponse, isReady: isFetchFinished, execute: fetchFeedback } = useAsyncState(
    () => {
        return doRequest({
            url: `/api/feedback/${route.params.id}/query/${route.params.queryId}`,
            method: 'GET',
            headers: {
...
watch(isFetchFinished, () => {
    const query = fetchResponse.value?.data ? fetchResponse.value?.data.query : [];
    queries.value = [ query ];
    isSubmitable.value = query.HumanFeedbackQuery.QueryStatusID < 3;
        ...
        <SectionMain>
            ...
            <SectionTitleLineWithButton :hasButton="false" :icon="mdiMessageAlertOutline" :title="pageTitle" main>
                <BaseButtons style="float:right">
                    <BaseButton v-if="isSubmitable" :label="$t('buttons.submit')" color="success" @click="onFeedbackSave" />
