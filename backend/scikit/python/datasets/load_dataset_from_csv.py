...
def main():
    ...
    parser.add_argument('-f2 ', "--orig_target_path", type=Path, required=False)
    ...
    X = np.load(args.orig_data_path, allow_pickle=True)
    ...
    if args.orig_target_path and args.target_path:
        y = np.load(args.orig_target_path, allow_pickle=True)
